...
def per_puan_hesapla(grup, okey_tas):
    ...
    # SERİ PER KONTROLÜ (Basit toplam)
...
def sirayi_degistir(chat_id):
    ...
    if res:
        ...
        # Sıra değişirken mevcut oyuncunun çekme durumunu sıfırla
        for p in players:
            if p['id'] == current_id: p['has_drawn'] = False
            
        next_id = p_ids[(p_ids.index(current_id) + 1) % len(p_ids)]
        cur.execute("UPDATE games SET current_turn_id = %s, players = %s WHERE chat_id = %s", 
                    (next_id, json.dumps(players), chat_id))
        conn.commit(); cur.close(); conn.close()
        return next_id
    return None
    ...
    if res and res[0]:
        ...
        for p in players:
            if p['id'] == player_id:
                ...
                p['has_drawn'] = True # Çekti olarak işaretle
                yeni_el = p['hand']
